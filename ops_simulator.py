...
def read_block(prompt):
    ...
    print(
        f"{C.GREY}  (Write your answer - commands, config or prose. "
...
def report_grading(hits, missed, points_awarded, points_max):
    ...
    print(
        f"{color}{C.BOLD}  Score for this question: "
...
class NetworkBus:
    ...
    def register(self, service_name, handler):
        ...
        print(
            f"{C.GREY}  [bus] service registered: "
    ...
    def call(self, service, endpoint, payload=None):
        ...
        print(
            f"{C.GREY}  [bus] {rid} POST /{service}/{endpoint} "
        ...
        print(
            f"{C.GREY}  [bus] {rid} <- {scolor}{status}{C.GREY} from {service}{C.RESET}"
        )
...
class ServiceModule:
    ...
    def numeric_question(self, prompt, validator, points, unit=""):
        ...
        if value is not None and validator(value):
            self.points += points
            print(
                f"{C.BGREEN}{C.BOLD}  Correct: {value:g} {unit} "
...
class ITDriftService(ServiceModule):
    ...
    def run(self, payload):
        banner(
            "MODULE 1 - IT-DRIFT",
            "Exam focus: June 23rd. Incidents first, then the oral exam board.",
            C.BRED,
        )
        wrap(
            """
        ...
        banner(
            "MODULE 1 COMPLETE",
            f"IT-Drift score: {self.points}/{self.max_points}",
            C.BRED,
        )
...
""",
    ...
    def scenario_split_brain(self):
        ...
        alert(
            "Two-node HA database cluster. Heartbeat link cut by a "
...
""",
    ...
    def scenario_bdr(self):
        ...
        self.numeric_question(
            "Q1 - Derive the maximum acceptable RTO in HOURS from the revenue numbers",
            lambda v: 0 < v <= 4,
            points=5,
            unit="hours",
        )
        ...
        self.free_question(
            ...
            [
                ...
                (
                    ...
                    [
                        r"nightly.{0,80}(24|rpo|loss|tab)",
                        r"24\s*(h|hour|timer)",
    ...
    def oral_exam_board(self):
        ...
        board = [
            ...
            (
                ...
                [
                    (
                        "Normal change -> CAB-godkendelse (assessment, authorization)",
            ...
            (
                ...
                [
                    ...
                    (
                        "aarlige surveillance audits + re-certificering hvert 3. aar",
                        [
                            r"surveillance",
                            r"3\.?\s*[aå]r",
                            r"re.?certificer",
            ...
            (
                ...
                [
                    ...
                    (
                        ...
                        [
                            r"test.{0,30}(restore|gendan)",
                            r"restore.{0,30}test",
                            r"utestet",
                            r"ikke\s+en\s+backup",
                        ],
                    ),
                ],
                8,
            ...
            (
                ...
                [
                    (
                        "ROI-formlen: (gevinst - investering) / investering x 100",
                        [
...
class FrontendService(ServiceModule):
    ...
    def run(self, payload):
        banner(
            "MODULE 2 - NEXT.JS FRONTEND",
            "Internship focus. Production debugging at scale.",
            C.BBLUE,
        )
        wrap(
            """
        ...
        banner(
            "MODULE 2 COMPLETE",
            f"Frontend score: {self.points}/{self.max_points}",
            C.BBLUE,
        )
    ...
    def scenario_memory_leak(self):
        ...
        alert(
            "Kubernetes: next-ssr pods OOMKilled every ~40 minutes "
...
""",
    ...
    def scenario_web_vitals(self):
        section("CASE 2: Core Web Vitals are failing - SEO traffic drops", C.BBLUE)
        code_block(
            """
...
""",
        ...
        self.free_question(
            "Q2 - Fix CLS. What is causing the 0.31 and what are the two fixes?",
            [
                (
    ...
    def scenario_isr(self):
        section("CASE 3: 50.000 product pages - the build takes 3 hours", C.BBLUE)
        wrap(
            """
    ...
    def scenario_hydration(self):
        section("CASE 4: 'Text content does not match server-rendered HTML'", C.BBLUE)
...
class BackendService(ServiceModule):
    ...
    def run(self, payload):
        banner(
            "MODULE 3 - SPRING BOOT BACKEND",
            "Internship focus. Enterprise Java under fire.",
            C.BGREEN,
        )
        wrap(
            """
        ...
        banner(
            "MODULE 3 COMPLETE",
            f"Backend score: {self.points}/{self.max_points}",
            C.BGREEN,
        )

    def scenario_hikari(self):
        section("CASE 1: Connection pool starvation at 09:00 every day", C.BGREEN)
        alert(
            "SQLTransientConnectionException: HikariPool-1 - Connection "
...
""",
            ...
            [
                ...
                (
                    ...
                    [
                        ...
                        r"little",
                        r"duration",
                        r"holder.{0,30}for\s+l",
                        r"ikke\s+pool\s*størrelse",
        ...
        print()
        info(
            "Now the config. HikariCP's own guidance: "
        ...
        info("(8 * 2) + 1 = 17. Anything from ~16-20 is defensible; 100 never was.")
        self.exact_command(
    ...
    def scenario_race(self):
        ...
        alert(
            "Finance reports: account #88231 balance = -7.250,00 kr. "
...
""",
        ...
        self.free_question(
            ...
            [
                (
                    ...
                    [r"for\s+update", r"pessimistic", r"pessimistisk", r"@lock"],
                ),
                (
            ...
            [
                ...
                (
                    ...
                    [
                        ...
                        r"flere\s+(instanser|pods)",
                        r"load\s*balanc",
                        r"distributed",
                        r"db.{0,60}(shared|arbiter|kilde)",
    ...
    def scenario_circuit_breaker(self):
        section("CASE 3: The downstream API that dragged you under", C.BGREEN)
        alert(
            "Your payment provider's API fails 80% of calls with 25s "
        ...
        self.free_question(
            ...
            [
                ...
                (
                    ...
                    [
                        r"slow.{0,80}(worse|værre)",
                        r"timeout.{0,60}(hold|binder)",
                        r"fast\s+fail",
                        r"25s",
                        r"hold.{0,30}(thread|resource)",
                    ],
                ),
                (
    ...
    def scenario_thread_dump(self):
        section("CASE 4: Production is frozen - read the thread dump", C.BGREEN)
        alert(
            "Order processing throughput: 0/sec. CPU: 3%. The JVM is "
...
""",
        ...
        self.exact_command(
            "Q1 - Name the two threads that are deadlocked (exact thread names).",
            [
            ...
            [
                (
                    ...
                    [
                        ...
                        r"samme\s+rækkefølge",
                        r"global\s+order",
                        r"always.{0,30}(first|før)",
                    ],
                ),
                (
...
class DatabaseService(ServiceModule):
    ...
    def run(self, payload):
        self.reset_state()
        banner(
            "MODULE 4 - POSTGRESQL OPERATIONS",
            "Internship focus. You are inside a simulated psql.",
            C.BYELLOW,
        )
        wrap(
            """
        ...
        banner(
            "MODULE 4 COMPLETE",
            f"Database score: {self.points}/{self.max_points}",
            C.BYELLOW,
        )
    ...
    def print_missions(self):
        ...
        for i, m in enumerate(self.MISSIONS, 1):
            mark = (
                f"{C.BGREEN}[done]{C.RESET}"
                if f"M{i}" in self.missions_done
                else f"{C.GREY}[open]{C.RESET}"
            )
    ...
    def repl(self):
        ...
        while True:
            ...
            if re.match(r"^(set|show)\b", low):
                print(
                    f"{C.GREY}   {('SET' if low.startswith('set') else 'SHOW')} "
                    f"acknowledged (simulated).{C.RESET}"
                )
                continue
            warn(
                "Command not recognized by the simulator. \\help lists "
        ...
        if "M3" not in self.missions_done:
            warn("M3 (deadlock) left unresolved - writes are still frozen. 0/10 pts.")
...
""",
    ...
    def handle_create_index(self, cmd):
        ...
        if table != "orders":
            warn(
                f"Index created on '{table}', but the slow queries hit "
                f"'orders'. No mission progress."
            )
            return
        if "attributes" in col:
            if method == "gin":
                self.indexes.add("idx_orders_attributes_gin (gin on orders.attributes)")
                if not self.gin_created:
                    self.gin_created = True
                    self.missions_done.add("M1")
                    self.points += 10
                    ok(
                        "CREATE INDEX (GIN on jsonb) ... done in 94s "
                        "(simulated). M1 logic complete: +10 pts. "
                        "Now verify with EXPLAIN + run the query."
                    )
            elif method in ("btree",):
                self.indexes.add(
                    "idx_orders_attributes_btree (btree on orders.attributes)"
                )
                warn(
                    "Index created - but a B-Tree on a jsonb column only "
                ...
                warn(
                    f"'{method}' is not a useful access method for jsonb "
                    f"containment here."
                )
        elif "created_at" in col:
            if method in ("btree",):
                self.indexes.add("idx_orders_created_at (btree on orders.created_at)")
                if not self.btree_created:
                    self.btree_created = True
                    self.missions_done.add("M2")
                    self.points += 6
                    ok(
                        "CREATE INDEX (B-Tree on created_at) ... done. "
                        "M2 complete: +6 pts. Range scans (>=, BETWEEN, "
                        "ORDER BY) are exactly what B-Trees are for."
                    )
            elif method == "gin":
                warn(
                    "GIN on a scalar timestamp gains you nothing - GIN "
                ...
                warn(
                    "Hash indexes support ONLY equality (=). Your query "
                    "is a range (>=). Seq Scan remains."
                )
        else:
            info(
                f"Index on orders({col}) created (simulated) - not "
...
""",
        ...
        warn(
            "Note pid 4821: 'idle in transaction' for 41 minutes. It is "
...
""",
    ...
    def handle_terminate(self, cmd):
        ...
        if pid == 4821:
            ...
            self.points += awarded
            ok(
                f"pg_terminate_backend(4821) -> t. The zombie transaction "
            ...
            alert(
                f"You terminated pid {pid} - a VICTIM. Its transaction "
...
def final_report(results):
    ...
    for r in results:
        ...
        print(
            f"  {C.WHITE}{r['service']:<14}{C.RESET} "
    ...
    hr("=", C.BMAGENTA)
    print(
        f"{C.BMAGENTA}{C.BOLD}  TOTAL: {total}/{total_max} ({pct_total:.0f}%) "
    ...
    try:
        while True:
            ...
            if response.get("status") == 200:
                body = response["body"]
                results = [r for r in results if r["service"] != body["service"]]
                results.append(body)
                info(
                    f"{body['service']} reported "
