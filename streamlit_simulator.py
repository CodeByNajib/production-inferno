...
def _steps_itdrift():
    ...
    s.append(
        dict(
            ...
            code=(
                "03:12:04 monitor  TLS handshake error: certificate has expired\n"
            ...
            groups=[
                ...
                (
                    "reads the dates or connects to :443",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "monitoring/alerting on cert expiry",
                    [
                        r"monitor",
                        r"alert",
                        r"blackbox",
                        r"prometheus",
                        r"expiry.{0,30}(warn|alert|check)",
                        r"30\s*d",
                    ],
                ),
                (
                ...
                (
                    "test the renewal path (dry-run, staging)",
                    [
                        r"dry.?run",
                        r"staging",
                        r"test.{0,20}renew",
            ...
            groups=[
                ...
                (
                    ...
                    [
                        r"http\s*flood",
                        r"cache.?bust",
                        r"random.{0,20}(query|param|string)",
                        r"unique.{0,20}(url|quer)",
                    ],
                ),
                (
    ...
    s.append(
        dict(
            ...
            groups=[
                ...
                (
                    "normalize/strip query strings or serve stale/static fallback",
                    [
                        r"normaliz",
                        r"strip.{0,15}quer",
                        r"static\s+fallback",
        ...
        dict(
            ...
            groups=[
                (
                    ...
                    [r"cost", r"cheap", r"asymmetr", r"attacker.{0,40}scal"],
                ),
                (
                    "database/stateful tier is the bottleneck and does not autoscale",
                    [r"database", r"\bdb\b", r"stateful", r"connection", r"bottleneck"],
                ),
                (
    ...
    s.append(
        dict(
            ...
            groups=[
                ...
                (
                    "no quorum / even number of nodes cannot form majority",
                    [
                        r"quorum",
                        r"majority",
                        r"even\s+number",
                        r"\b2\s+nodes?\b.{0,60}(cannot|kan ikke)",
                    ],
                ),
                (
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "STOP the bleeding: fence/isolate one node or block writes at the LB first",
                    [
                        r"fence",
                        r"stonith",
                        r"stop.{0,25}writ",
                ...
                (
                    "choose ONE authoritative node (by txn/WAL analysis)",
                    [
                        r"authoritative",
                        r"source\s+of\s+truth",
                        r"pick\s+one",
                        r"compare.{0,30}(data|timeline|wal)",
                    ],
                ),
                (
                    "reconcile/merge or replay diverged writes before resuming",
                    [r"reconcil", r"merge", r"replay", r"diverge"],
                ),
                (
                    "rebuild the loser as a fresh replica from the winner",
                    [r"rebuild", r"re.?sync", r"basebackup", r"re.?clone", r"replica"],
                ),
            ],
            points=10,
            threshold=3,
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "odd number of voters: 3 nodes, or 2 nodes + witness/quorum device",
                    [
                        r"\b3\b.{0,20}(node|voter)",
                        r"odd",
                        r"witness",
                        r"quorum\s+(device|disk)",
                        r"arbiter",
                        r"tie.?breaker",
                    ],
                ),
                ...
                (
                    ...
                    [
                        ...
                        r"separate.{0,25}(heartbeat|network)",
                        r"dual.{0,10}link",
                    ],
                ),
                (
                    "majority-based promotion (Raft/Paxos/Patroni/etcd)",
                    [
                        r"raft",
                        r"paxos",
                        r"patroni",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "MAO = Maximum Acceptable Outage — den samlede maksimale nedetid",
                    [
                        r"maximum\s+acceptable\s+outage",
                        r"samlede?.{0,40}nedetid",
                        r"maks.{0,30}(outage|nedetid)",
                        r"total.{0,30}(downtime|outage)",
                    ],
                ),
                (
                ...
                (
                    "defineres i BIA — høj kritikalitet = lavere RPO/RTO",
                    [
                        r"\bbia\b",
                        r"business\s+impact",
                        r"kritikalitet",
            ...
            groups=[
                ...
                (
                    ...
                    [
                        ...
                        r"log\s+shipping",
                        r"stream",
                        r"continuous\s+replicat",
                    ],
                ),
                (
                    "nightly backups violate RPO (up to 24h data loss)",
                    [r"nightly.{0,80}(24|rpo|loss)", r"24\s*(h|hour|timer)"],
                ),
                (
    ...
    board = [
        (
            ...
            [
                ...
                ("Deliver & Support + Improve", [r"deliver", r"support", r"improve"]),
                (
                    "guiding principles (focus on value, iterate, collaborate, keep simple, automate…)",
                    [
        ...
        (
            ...
            [
                ...
                (
                    "Linje 1 = service desk løser simple/kendte problemer",
                    [
                        r"linje\s*1",
                        r"service\s*desk",
                        r"f[oø]rste.?linje",
                ...
                (
                    "P1 Critical: immediate response, resolution ~1 time",
                    [
                        r"immediate",
                        r"straks",
                        r"1\s*(time|hour)",
        ...
        (
            ...
            [
                ...
                (
                    "Standard (pre-godkendt, lav risiko) og Emergency (ECAB)",
                    [r"standard\s+change", r"pre.?godkendt", r"emergency", r"ecab"],
                ),
                (
                    "RFC + back-out/rollback-plan + test",
                    [
                        r"\brfc\b",
                ...
                (
                    "7 R'er: Raised, Reason, Return, Risks, Resources, Responsible, Relationship",
                    [
                        r"raised",
                        r"reason",
                        r"return",
        ...
        (
            ...
            [
                (
                    "scope: hvilke services/systemer er dækket",
                    [r"scope", r"services?\b", r"omfang", r"hvilke\s+system"],
                ),
                (
                    "målbare mål: oppetid (fx 99,9%) + response/resolution tider",
                    [
                        r"99[.,]9",
                        r"oppetid",
                        r"uptime",
                ...
                (
                    ...
                    [r"kpi", r"m[aå]l(ing|es)", r"rapport", r"metric", r"monitor"],
                ),
                (
                ...
                (
                    "SLA=kunde; OLA=interne teams; UC=ekstern leverandør — skrappere end SLA'en",
                    [
                        r"ola",
                        r"underpinning",
                        r"\buc\b",
        ...
        (
            ...
            [
                (
                    "Identification = du hævder hvem du er (brugernavn)",
        ...
        (
            ...
            [
                (
                    "ISMS = system af politikker/processer/kontroller; 27001 = standarden/kravene",
                    [r"isms", r"management\s+system", r"ledelsessystem"],
                ),
                (
                    "SoA = Statement of Applicability: valgte/fravalgte kontroller og HVORFOR",
        ...
        (
            ...
            [
                ...
                (
                    ...
                    [r"surveillance", r"3\.?\s*[aå]r", r"re.?certificer", r"[aå]rlig"],
                ),
                (
                    "minor = isoleret fejl; major = systemisk svigt, kan blokere certificering",
                    [
        ...
        (
            ...
            [
                ...
                (
                    "forebyg: Clear Desk Policy, besøgsregler, adgangskontrol",
                    [
                        r"clear\s*desk",
                        r"bes[oø]gs",
                        r"printer",
                ...
                (
                    "GDPR-begreber: DSAR, retten til at blive glemt, privacy by design",
                    [
                        r"dsar",
                        r"blive\s+glemt",
                        r"forgotten",
        ...
        (
            ...
            [
                (
                    "agenten læser data, pakker/krypterer og sender til storage; styres af Backup Manager",
                    [
                ...
                (
                    "inkrementel backup — kun ændringer siden sidst",
                    [r"inkrement", r"increment", r"kun.{0,30}[aæ]ndr"],
                ),
                (
                    "SAN = block-level, dedikeret netværk; NAS = file-level",
                    [
                        r"san.{0,80}block",
                        r"block.{0,60}san",
                        r"nas.{0,80}file",
                ...
                (
                    ...
                    [r"test.{0,30}(restore|gendan)", r"utestet", r"ikke\s+en\s+backup"],
                ),
            ],
            8,
            3,
        ...
        (
            ...
            [
                ...
                (
                    "Level 3 = processer defineret på ORGANISATIONSNIVEAU",
                    [
                        r"organisationsniveau",
                        r"org.{0,20}wide",
                        r"samme\s+(regler|processer)",
                    ],
                ),
                (
                    "man må IKKE springe niveauer over",
                    [
                        r"ikke.{0,30}spring",
                        r"cannot\s+skip",
                        r"fundament",
                ...
                (
                    "plan: dokumenter processer, ITIL som ramme, mål med metrics",
        ...
        (
            ...
            [
                (
                    "Type 1 = bare-metal, direkte på hardware (ESXi, Hyper-V, KVM)",
                    [
                ...
                (
                    "Type 2 = hosted, oven på et OS (VirtualBox, VMware Workstation)",
                    [
                ...
                (
                    "fordele: konsolidering, snapshots, skalering, failover/HA",
                    [r"konsolider", r"snapshot", r"skaler", r"failover"],
                ),
                (
                    "noisy neighbor + hypervisor-overhead",
                    [r"noisy\s+neighbor", r"overhead", r"deler?\s+ressourcer"],
                ),
                (
                    "fysisk ved: HPC, kæmpe databaser, compliance",
                    [
                        r"hpc",
                        r"high.?performance",
        ...
        (
            ...
            [
                (
                    "ROI-formlen: (gevinst − investering) / investering × 100",
                    [
                        r"gevinst\s*-\s*invest",
                        r"\(.{0,30}-.{0,30}\)\s*/\s*invest",
                        r"return\s+on\s+investment",
                    ],
                ),
                (
                    "regnestykket: ROI = 20% / break-even under 2 år",
                    [r"40\.?000", r"20\s*%", r"break.?even", r"240"],
                ),
                (
                    "ROI → business case for ledelsen; Change Management → kontrolleret gennemførelse",
                    [
                        r"roi.{0,120}(ledelse|business\s*case|godkend)",
                        r"change\s*management.{0,120}(kontrol|risiko|back.?out)",
                    ],
                ),
            ],
            8,
        ...
        (
            ...
            [
                (
                    "IaaS = rå infrastruktur/VM'er (lejer hardware)",
                    [
                ...
                (
                    "PaaS = platform/miljø til at udvikle apps",
                    [
                ...
                (
                    "SaaS = færdigt program via browser (M365)",
                    [
                ...
                (
                    "Shared Responsibility: ansvaret deles forskelligt pr. lag",
                    [
                        r"shared\s+responsibility",
                        r"delt\s+ansvar",
                        r"provider.{0,60}kunde",
                    ],
                ),
            ],
            8,
        ...
        (
            ...
            [
                ...
                (
                    ...
                    [r"\bci\b.{0,120}(auto|test|integr)", r"continuous\s+integration"],
                ),
                (
                    "Continuous Delivery = deployerbar, men MANUEL godkendelse",
                    [
                ...
                (
                    "Continuous Deployment = fuldt automatisk til produktion",
        ...
        (
            ...
            [
                ...
                (
                    "Continued Business Justification: projektet skal ALTID kunne retfærdiggøres",
                    [
                        r"business\s+justification",
                        r"retf[aæ]rdigg",
                        r"forretningsm[aæ]ssig",
                    ],
                ),
                (
                    "Manage by Exception: styregruppen involveres kun ved afvigelser",
                    [
                ...
                (
                    "Learn from Experience, Roles & Responsibilities, Stages, Focus on Products, Tailor",
                    [
        ...
        (
            ...
            [
                (
                    "VLAN-segmentering begrænser spredning/skadeomfang",
                    [
                        r"vlan",
                        r"segment",
                        r"begr[aæ]ns.{0,40}(spredning|angreb|skade)",
                        r"adskil",
                    ],
                ),
                (
                    "hub=lag 1; switch=lag 2 (MAC); router=lag 3 (IP)",
                    [
                ...
                (
                    "IDS opdager / IPS blokerer mistænkelig trafik",
                    [r"ids", r"ips", r"intrusion", r"honeypot"],
                ),
                (
                    "DMZ isolerer offentligt eksponerede services fra internt net",
        ...
        (
            ...
            [
                (
                    "AD-konto oprettes (PowerShell New-ADUser) i den rette OU",
                    [
                        r"new-aduser",
                        r"powershell",
                        r"csv",
                ...
                (
                    "GPO pusher konfiguration (password-politik, firewall, software)",
                    [
                        r"gpo",
                        r"group\s*policy",
                        r"password.?politik",
                ...
                (
                    "MDM (fx Intune) enroller enheder og håndhæver compliance",
                    [r"mdm", r"intune", r"enroll", r"complian"],
                ),
                (
                    "PoLP/RBAC: kun nødvendige rettigheder via grupper",
                    [r"polp", r"least\s+privilege", r"rbac", r"grupper"],
                ),
                (
                    "MDM = hele enheden; MAM = kun firma-apps/data (BYOD, remote wipe)",
                    [
                        r"mam",
                        r"byod",
                        r"remote\s*wipe",
        ...
        (
            ...
            [
                (
                    "symmetrisk = en delt nøgle (hurtig); asymmetrisk = public/private key (HTTPS)",
                    [
                        r"symmetrisk.{0,120}(en|delt|samme)\s*n[oø]gle",
                        r"asymmetrisk.{0,120}(public|private)",
                        r"https",
                    ],
                ),
                (
                    "hashing er ENVEJS — kan ikke gendannes; sammenlign hash af input",
                    [
                        r"envejs",
                        r"one.?way",
                        r"ikke\s+gendan",
                        r"sammenlign.{0,40}hash",
                    ],
                ),
                (
                    "Diffie-Hellman: sikker nøgleudveksling over usikker kanal",
                    [
                        r"diffie",
                        r"n[oø]gleudveksling",
                        r"usikker\s+(kanal|forbindelse)",
                        r"key\s+exchange",
                    ],
                ),
                (
                    "2FA = Something you HAVE / ARE / KNOW",
                    [
                        r"have",
                        r"\bare\b",
                        r"know",
    ...
    for i, (prompt, groups, pts, thresh) in enumerate(board):
        step = dict(
            kind="question", prompt=prompt, groups=groups, points=pts, threshold=thresh
        )
        if i == 0:
            step["scenario"] = "THE ORAL EXAM BOARD — EK/KEA IT-Drift pensum"
            step["info"] = (
                "To censorer (JHB og Oskar Tuska). Svar i hele sætninger — dansk eller engelsk."
            )
        s.append(step)
...
def _steps_frontend():
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "module-scope Map lives for the entire process lifetime (evaluated once, singleton)",
                    [
                        r"module.{0,30}(scope|level)",
                        r"top.?level",
                        r"global",
                        r"process\s+lifetime",
                        r"evaluated\s+once",
                        r"singleton",
                    ],
                ),
                (
                    "unbounded growth: 600k keys × 120 KB, no eviction/TTL",
                    [
                        r"unbounded",
                        r"no\s+(eviction|ttl|limit)",
                        r"grows?\s+forever",
                        r"never\s+(evict|clear|remov)",
                    ],
                ),
                (
                    "dev restarts/HMR constantly so it never accumulates; prod is long-lived",
                    [
                        r"dev.{0,80}(restart|recompil|hot\s*reload|hmr)",
                        r"long.?lived",
                        r"shared\s+across\s+request",
                    ],
                ),
            ],
            points=9,
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "bounded cache: LRU with max size and/or TTL (e.g. lru-cache)",
                    [
                        r"\blru\b",
                        r"ttl",
                        r"max.{0,12}(size|entries)",
                        r"bounded",
                        r"evict",
                    ],
                ),
                (
                    "externalize: Redis/CDN cache shared across pods",
                    [
                        r"redis",
                        r"memcach",
                        r"external\s+cache",
                ...
                (
                    "per-request memoization (React cache() / request scope) instead of process scope",
                    [
                        r"react\s+cache",
                        r"\bcache\(\)",
                        r"per.?request",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "next/image with priority (preload + optimized format/size)",
                    [r"next/image", r"<image", r"\bpriority\b", r"preload"],
                ),
                (
                    "modern format + responsive sizes (AVIF/WebP, srcset/sizes)",
                    [r"avif", r"webp", r"srcset", r"\bsizes\b", r"compress", r"resize"],
                ),
                (
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "late-injected banner shifts layout / images without dimensions",
                    [
                        r"banner",
                        r"inject",
                        r"shift",
                ...
                (
                    "reserve space: fixed-size container/skeleton/placeholder or min-height",
                    [
                        r"reserve",
                        r"placeholder",
                        r"skeleton",
                ...
                (
                    "set width/height so the browser can pre-allocate the box",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "dynamic import / next/dynamic / code splitting (lazy load on the 2%)",
                    [
                        r"dynamic\s*import",
                        r"next/dynamic",
                        r"import\(",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "revalidate = 300 (export const revalidate / getStaticProps revalidate / fetch next.revalidate)",
                    [
                        r"revalidate\s*[:=]\s*300",
                        r"revalidate\s*[:=]\s*\d+",
                        r"next:\s*\{\s*revalidate",
                    ],
                ),
                (
                    "static generation as the base (generateStaticParams / getStaticProps / getStaticPaths)",
                    [
    ...
    s.append(
        dict(
            ...
            groups=[
                ...
                (
                    "triggered by a CMS webhook hitting a secured route handler/API route",
                    [
                        r"webhook",
                        r"api\s*route",
                        r"route\s*handler",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "generateStaticParams returns subset + dynamicParams=true (or fallback:'blocking' in Pages Router)",
                    [
                        r"dynamicparams",
                        r"fallback",
                        r"blocking",
                ...
                (
                    "first visitor triggers SSR/on-demand generation, result is cached for everyone after",
                    [
                        r"first\s+(visit|request).{0,90}(generat|render|ssr)",
                        r"on.?demand.{0,40}(generat|render)",
                        r"cached?\s+(after|for)",
                    ],
                ),
            ],
            points=7,
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "hydration = React attaches to server HTML and expects IDENTICAL first client render",
                    [
                        r"hydrat",
                        r"attach",
                        r"match.{0,40}server",
                ...
                (
                    "Date.now()/time differs (server render time vs client render time)",
                    [r"date\.now", r"time\s+diff", r"clock", r"render\s+time"],
                ),
                (
                    "toLocaleTimeString depends on locale/timezone — server (UTC) vs user's browser",
                    [r"locale", r"timezone", r"tidszone", r"utc", r"tolocale"],
                ),
            ],
            points=9,
            threshold=3,
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "two-pass render: useState+useEffect / 'mounted' flag — render time-dependent parts only after mount",
                    [
                        r"useeffect",
                        r"usestate",
                        r"mounted",
                ...
                (
                    "make output deterministic: compute greeting/seed on the server and pass as prop",
                    [
                        r"prop",
                        r"deterministic",
                        r"seed",
                        r"server.{0,40}(compute|decide)",
                        r"samme\s+(v[aæ]rdi|input)",
                    ],
                ),
                (
                    "suppressHydrationWarning only for a single unavoidable node — never as a blanket fix",
                    [
...
def _steps_backend():
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "slow external HTTP call happens INSIDE @Transactional, so each request holds a DB connection for ~8s",
                    [
                ...
                (
                    "pool size is not the bottleneck — hold TIME is (Little's Law)",
                    [
                        r"hold.{0,12}time",
                        r"little",
                        r"duration",
                        r"not.{0,20}pool\s+size",
                    ],
                ),
                (
                    "100 conns can also overload Postgres itself (context switching, lock contention)",
                    [
                        r"100.{0,80}(overload|too\s+many|postgres)",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "move the external call OUTSIDE the transaction (call tax API first, then open a short transaction)",
                    [
                        r"(outside|out\s+of|f[oø]r|before).{0,40}transak?c",
                        r"split.{0,40}transac",
                        r"remove.{0,20}@?transactional",
                    ],
                ),
                (
                    "keep transactions short: only DB work inside, set timeouts on the HTTP client",
                    [r"short\s+transac", r"timeout", r"kun\s+db", r"only\s+db"],
                ),
            ],
            points=7,
            threshold=1,
    ...
    s.append(
        dict(
            ...
            groups=[
                ...
                (
                    "a value in the seconds-to-a-minute range",
                    [
        ...
        dict(
            ...
            groups=[
                ...
                (
                    ...
                    [
                        r"both.{0,60}read",
                        r"read.{0,60}(before|f[oø]r).{0,30}(writ|save)",
                        r"same\s+balance",
                    ],
                ),
                (
                    "both pass the check, both subtract, second save wins → overdraw",
                    [
                        r"both\s+pass",
                        r"second.{0,30}(save|write)",
                        r"overwrit",
                        r"overdraw",
                        r"negativ",
                    ],
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "pessimistic locking: SELECT ... FOR UPDATE / @Lock(PESSIMISTIC_WRITE)",
                    [r"for\s+update", r"pessimistic", r"@lock"],
                ),
                (
                    "optimistic locking: @Version column, retry on OptimisticLockException",
                    [r"optimistic", r"@version", r"version\s+(col|felt|field)"],
                ),
                (
                    "atomic conditional UPDATE: SET balance = balance - ? WHERE balance >= ?",
                    [
                        r"update.{0,80}balance\s*-\s*",
                        r"where\s+balance\s*>=",
                        r"atomic\s+update",
                        r"conditional\s+update",
                    ],
                ),
                (
                    "trade-off: pessimistic blocks (safe, lower throughput); optimistic scales but needs retry",
                    [
                        r"(pessimi|lock).{0,120}(block|throughput|contention|venter)",
                        r"optimi.{0,120}(retry|conflict|skaler|scale)",
                        r"trade.?off",
                    ],
                ),
            ],
        ...
        dict(
            ...
            groups=[
                (
                    ...
                    [
                        r"one\s+jvm",
                        r"single\s+(jvm|instance|process)",
                        r"within.{0,20}(jvm|process)",
                    ],
                ),
                (
                    "production runs multiple instances/pods — the DB is the only shared arbiter",
                    [
                        r"multiple\s+(instance|pod|node)",
                        r"load\s*balanc",
                        r"distributed",
                        r"database.{0,40}(eneste|only|shared|arbiter)",
                    ],
                ),
            ],
            points=6,
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "shared, finite thread pool (Tomcat workers) exhausted by threads parked waiting on slow dependency",
                    [
                        r"thread\s*pool",
                        r"tomcat",
                        r"worker",
                ...
                (
                    "slow is worse than down: 25s timeouts hold resources; a fast failure would release them",
                    [
                        r"slow.{0,80}(worse|v[aæ]rre)",
                        r"timeout.{0,60}(hold|binder)",
                        r"fast\s+fail",
                        r"25s",
                    ],
                ),
                (
                    "no bulkhead/isolation between dependency calls and the rest of the app",
                    [r"bulkhead", r"isolat", r"shared\s+resource"],
                ),
            ],
            points=8,
            threshold=2,
    ...
    s.append(
        dict(
            ...
            groups=[
                ...
                (
                    "half-open probe calls (permitted-number-of-calls-in-half-open-state)",
                    [r"half.?open", r"permitted.?number"],
                ),
                (
                    "plus a TIMEOUT (TimeLimiter/connect+read timeout) — breaker without timeout is half a fix",
        ...
        dict(
            ...
            groups=[
                ...
                (
                    ...
                    [r"half.?open", r"probe", r"test\s+calls"],
                ),
                (
                    "fallback: queue payment for retry / alternative provider / graceful degradation",
                    [
                        r"fallback",
                        r"queue",
                        r"retry\s+later",
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "order-worker-3 holds Ledger(76aa1), waits for Inventory(76ab2)",
                    [
                        r"order.?worker.?3.{0,140}(hold|locked|76aa1|ledger).{0,160}(wait|76ab2|inventory)"
                    ],
                ),
                (
                    "inventory-sync-1 holds Inventory(76ab2), waits for Ledger(76aa1) — a cycle",
                    [
                ...
                (
                    "order-worker-1 is WAITING/parked on a queue = idle and normal, not BLOCKED on a monitor",
                    [
    ...
    s.append(
        dict(
            ...
            groups=[
                (
                    "global consistent lock ORDERING (always Ledger before Inventory, everywhere)",
                    [
                        r"lock\s+order",
                        r"consistent\s+order",
                        r"global\s+order",
                        r"always.{0,30}(first|f[oø]r)",
                    ],
                ),
                (
                    "tryLock with timeout + back off (ReentrantLock) instead of synchronized",
                    [r"trylock", r"timeout", r"back.?off", r"reentrantlock"],
                ),
                (
                    "shrink/redesign: smaller critical sections, one lock, or a queue/single-writer design",
                    [
                        r"smaller\s+critical",
                        r"single\s+lock",
                        r"queue",
                        r"single.?writer",
                        r"redesign",
    ...
    return [
        ...
        dict(
            ...
            groups=[
                ...
                (
                    "primary: max_wal_senders > 0 (+ replication slot / pg_hba replication entry)",
                    [r"max_wal_senders", r"replication\s+slot", r"pg_hba"],
                ),
                (
                    "standby: hot_standby = on + primary_conninfo (+ standby.signal)",
                    [r"hot_standby", r"primary_conninfo", r"standby\.signal"],
                ),
                (
                    "caveat: ASYNC by default → replication lag, read-your-own-writes can fail",
                    [
                        r"lag",
                        r"async",
                        r"asynkron",
        ...
        dict(
            ...
            groups=[
                ...
                (
                    ...
                    [r"join\s+customers", r"customers\s+c", r"c\.name", r"customers\."],
                ),
            ],
            points=12,
            threshold=3,
...
def _repl_explain_range(btree_created):
    if btree_created:
        return (
            "Aggregate → Index Only Scan using idx_orders_created_at\n"
...
def _repl_process(cmd, rs):
    ...
    if low in (r"\d", r"\dt", r"\d orders"):
        out.append(
            (
                ...
                "Indexes: " + ", ".join(sorted(rs["indexes"])),
                "code",
            )
        )
        return out
    ...
    if re.match(r"^explain", low):
        if "attributes" in low or "@>" in cmd:
            ...
            out.append(
                ("Seq Scan on orders  (cost=0.00..312044.10 rows=12400000)", "code")
            )
        return out
    ...
    if re.match(r"^create\s+index", low):
        ...
        if table != "orders":
            out.append(
                (
                    f"Index created on '{table}', but slow queries hit 'orders'. No mission progress.",
                    "warn",
                )
            )
            ...
            if method == "gin":
                rs["indexes"].add(
                    "idx_orders_attributes_gin (gin on orders.attributes)"
                )
                if not rs["gin_created"]:
                    ...
                    out.append(
                        (
                            "CREATE INDEX (GIN on jsonb) done. M1 complete: +10 pts. Verify with EXPLAIN.",
                            "ok",
                        )
                    )
                else:
                ...
                out.append(
                    (
                        "B-Tree on jsonb only supports equality on the WHOLE document. The @> operator CANNOT use it. Think: operator class.",
                        "warn",
                    )
                )
            ...
            if method == "btree":
                ...
                if not rs["btree_created"]:
                    ...
                    out.append(
                        (
                            "CREATE INDEX (B-Tree on created_at) done. M2 complete: +6 pts.",
                            "ok",
                        )
                    )
                else:
                ...
                out.append(
                    (
                        "GIN on a scalar timestamp gains nothing. Range predicates on scalars want a B-Tree.",
                        "warn",
                    )
                )
            else:
                out.append(
                    (
                        "Hash indexes support ONLY equality (=). Your query is a range (>=). Seq Scan remains.",
                        "warn",
                    )
                )
        return out
        ...
        if rs["deadlock_resolved"]:
            ...
            out.append(
                (
                    ...
                    "⚠️  pid 4821: 'idle in transaction' for 41 minutes — holding locks but doing NOTHING.",
                    "warn",
                )
            )
        return out
    ...
    if "pg_terminate_backend" in low or "pg_cancel_backend" in low:
        ...
        if rs["deadlock_resolved"]:
            ...
            out.append(
                (
                    f"pg_terminate_backend(4821) → t. The zombie transaction rolled back, locks released. "
                    f"Pids 5102/5311/5409 completed within 2s. M3 complete: +{awarded} pts.",
                    "ok",
                )
            )
            ...
            out.append(
                (
                    f"You terminated pid {pid} — a VICTIM. Its transaction rolled back (lost work for a customer), "
                    f"and the queue re-formed behind the real culprit (pid 4821, still idle in transaction). -3 pts on M3.",
                    "error",
                )
            )
        else:
    ...
    if re.match(r"^select", low):
        ...
        out.append(
            (
                f"Time: {ms} ms"
                + (
                    " ← still reading the whole table. EXPLAIN it." if ms > 1000 else ""
                ),
                style,
            )
        )
    ...
    out.append(
        (
            "Command not recognized. Supported: EXPLAIN, CREATE INDEX, SELECT * FROM pg_stat_activity, SELECT * FROM pg_locks, SELECT pg_terminate_backend(<pid>), \\d, \\di, \\q",
            "warn",
        )
    )
    return out
...
def _render_repl(step):
    ...
    cmd = st.text_input(
        "nordshop-prod=#",
        key=f"repl_cmd_{len(st.session_state.repl_history)}",
        placeholder="Type a command (\\q to exit REPL)",
    )
...
Select a module from the sidebar to begin.
    ...
    if idx >= len(steps):
        ...
        if pct < 60:
            st.info(
                "Re-run the module. Mastery means the post-mortems contain nothing you didn't already say yourself."
            )
    ...
    if st.session_state.submitted and st.session_state.feedback:
        ...
        if "postmortem_title" in step:
            with st.expander(
                f"📋 Post-mortem: {step['postmortem_title']}", expanded=False
            ):
    ...
    if kind == "numeric":
        val = st.number_input(
            "Your answer:", value=0.0, step=0.5, format="%.1f", key=f"num_{idx}"
        )
        unit = step.get("unit", "")
        if unit:
